...
A key processing step in order for accurate motor unit activation predictions is normalizing the raw data to the maximum of each muscle. As shown in the plot below, this can be difficult with the noise and artifacts even after an outlying peak-removing step.  
